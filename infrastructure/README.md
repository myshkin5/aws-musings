...
---|---|---|---
 `SecondOctet` | `SECOND_OCTET` | Yes / `0` | The second octet of CIDR of the entire VPC. The first octet is always `10` for a full CIDR of `10.0.0.0/16` (assuming `SecondOctet` is set to `0`).
...
---|---|---|---
 ...
 `FullyQualifiedExternal ParentDNSZone` (without space) | `FULLY_QUALIFIED_EXTERNAL _PARENT_DNS_ZONE` (without space) | No, `vkzone.net` | <a name="fully-qualified-external-parent-dns-zone">The</a> public DNS zone configured in Route 53. If not specified, no public DNS record will be created for the jump box.
 ...
 `SecondOctet` | `SECOND_OCTET` | Yes / `0` | See the [VPC stack](#vpc) above.
 `VPCId` | `VPC_ID` | Yes | See the [VPC stack](#vpc) above.
